...
class RateLimited(Exception):
    def __init__(self, reset: str = "", retry_after: str = "") -> None:
        ...
        self.minutes = _reset_minutes(reset) or _retry_after_minutes(retry_after)
...
def _retry_after_minutes(value: str) -> int:
    try:
        seconds = int(value)
    except ValueError:
        return 0
...
def _rate_limit_error(exc: urllib.error.HTTPError) -> RateLimited | None:
    if exc.code == 429 or (exc.code == 403 and (
            exc.headers.get("x-ratelimit-remaining") == "0"
            or exc.headers.get("retry-after"))):
        return RateLimited(exc.headers.get("x-ratelimit-reset", ""),
...
def _fetch_part(url: str, part: Path, on_progress: Callable[[float], None] | None,
    ...
    headers = {"User-Agent": "Throwback", "Accept": "application/octet-stream"}
    if offset:
