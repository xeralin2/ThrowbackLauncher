...
def ensure_wine(reporter: Reporter | None = None,
    ...
    try:
        ...
        fetch_to(asset_url, archive, on_progress=reporter.progress, cancelled=cancelled)
...
def is_radmin_installer(path: Path) -> bool:
    p = Path(path)
    try:
...
class Session:
    ...
    def run(self, installer: Path | None, reporter: Reporter,
        ...
        try:
            ...
            reporter.progress(0.92)
            error = self._net_ready()
            if error:
                reporter.fail(log.fail("Routing failed", error))
                return
            self._launch_gui(reporter)
            reporter.progress(1.0)
            reporter.succeed(f"Radmin VPN running, {self._vpn_ip}")
            on_running()
            self._wait_exit()
        finally:
            self._teardown()
    ...
    def _ensure_installed(self, installer: Path | None, reporter: Reporter) -> bool:
        ...
        if installer is None or not Path(installer).is_file():
            ...
            return False
        reporter.update("Installing Radmin VPN")
        ...
        self._scrub_ndis()
        self._reg_import([
            (r"HKLM\SYSTEM\CurrentControlSet\Services\RvControlSvc", "Start", "REG_DWORD", "4"),
        ])
        _wineserver_stop(self._env)
        reporter.succeed("Radmin VPN installed")
        return True
    ...
    def _load_mac(self) -> None:
        ...
        try:
            ...
            raise OSError(f"cannot write {_MAC_RAW}, remove it and retry") from e
    ...
    def _start_bridge(self, reporter: Reporter) -> bool:
        reporter.update("Starting network bridge")
    ...
    def _start_service(self, reporter: Reporter) -> bool:
        reporter.update("Starting service")
        ...
        for i in range(150):
            ...
            if self._service_proc.poll() is not None:
                reporter.fail(log.fail("Radmin VPN service exited",
                                       f"exit={self._service_proc.returncode}"))
                return False
        reporter.fail(log.fail("Radmin VPN service never became ready", "30s timeout"))
        return False
    ...
    def _launch_gui(self, reporter: Reporter) -> None:
        reporter.update("Starting Radmin VPN")
        reporter.progress(0.96)
        env = dict(self._env)
