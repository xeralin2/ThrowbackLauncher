...
SEASON_RUNNING = "Close this season first"
...
def season_pids(key: str) -> list[int]:
    roots = _library_roots()
    names = {key, hm_folder_name(key)}
    return [
        pid for pid in running_game_pids() if _game_folder(pid, roots) in names
    ]
...
def _delete_folder(folder: Path) -> dict | None:
    try:
        ...
        return {"ok": False, "message": log.fail("Could not remove the game files", e)}
    return None
...
def _delete_prefix(key: str) -> dict | None:
    ...
    if prefix.exists():
        try:
            ...
            return {"ok": False, "message": log.fail("Could not remove the Proton prefix", e)}
    return None
...
def uninstall(key: str, hm: bool) -> dict:
    ...
    prefix = PREFIX_DIR / key
    if folder is None and not prefix.exists():
