...
from core.heatedmetal import apply_hm, remove_hm_files
...
def _ensure_runtime(is_hm: bool, reporter: Reporter | None = None) -> Path | None:
    dd = ensure_depotdownloader(reporter)
    if dd is None:
        return None
    if not is_hm and not ensure_tl(reporter):
        return None
    return dd
...
def _apply_install(target: Path, download: dict, is_hm: bool, username: str,
    ...
    try:
        ...
        return False
    reporter.succeed("ThrowbackLoader applied")
    return True
...
class DownloadController(QObject):
    ...
    disk_space_required = Signal(float, float)
    error = Signal(str)
    ...
    def set_peers(self, updater: QObject, settings_bridge: QObject, uninstaller: QObject,
        ...
        uninstaller.item_done.connect(lambda *_: self._start_next())
    ...
    def _conflicts(self, season_key: str) -> bool:
        if self._uninstaller.busy_key() == season_key:
            ...
            return True
        if self._updater.property("busy"):
            self.error.emit("Updates are running")
            return True
    ...
    def _accept(self, season_key: str, resume: tuple) -> dict | None:
        ...
        if self._deleting_key == season_key:
            self.error.emit("Deleting files")
            return None
    ...
    def _enqueue(self, season_key: str, enable_hm: bool, library: str) -> None:
        if not self._running:
            ...
            return
        if (season_key, enable_hm) == (self._active_key, self._enable_hm) or any(
            item[0] == season_key and item[1] == enable_hm for item in self._queue
        ):
            return
    ...
    def _enter_paused_from_queue(self) -> bool:
        ...
        if partial is not None:
            self._target = partial
        else:
            root = Path(library).resolve() if library else default_library()
            self._target = root / variant_folder(key, hm)
    ...
    def _start_next(self) -> None:
        ...
        while self._queue and not self._running and self._pending_request is None:
            ...
            if (season_key == self._deleting_key or self._updater.property("busy")
    ...
    def _start(self, season_key: str, enable_hm: bool, library: str,
        ...
        if download is None:
            return
        partial = partial_path(season_key, enable_hm)
        if partial is not None:
            target = partial
            if library and Path(library).resolve() != target.parent:
            ...
            if root not in libraries():
                ...
                return
            if not root.exists():
                self.error.emit("Library folder is missing")
                return
            target = root / variant_folder(season_key, enable_hm)
        ...
        if not skip_disk_check:
            ...
            if free < required:
                ...
                self._persist_queue()
                self.disk_space_required.emit(download["size_gb"], free_gb)
                return
    ...
    def _delete_partial(self, season_key: str, hm: bool) -> None:
        if self._running and (season_key, hm) == (self._active_key, self._enable_hm):
            self.partial_deleted.emit(
                season_key, hm, False, "Wait for the active download to finish"
            )
            return
        if self._deleting_key is not None:
            self.partial_deleted.emit(season_key, hm, False, "Deleting files")
            return
        installed = installed_path(season_key, hm)
        folder = variant_folder(season_key, hm)
        targets = [
            path
        ...
        if not targets:
            self.partial_deleted.emit(season_key, hm, False, "Nothing to delete")
            return
        ...
        def work() -> None:
            try:
                ...
                self._deleted_in.emit(season_key, hm, True, f"{code} download removed")
    ...
    def _verify(self, season_key: str, hm: bool) -> None:
        if self._running:
            if (season_key, hm) == (self._active_key, self._enable_hm) or any(
                item[0] == season_key and item[1] == hm for item in self._queue
            ):
                return
        ...
        if is_season_running(season_key):
            self.error.emit("Close this season first")
            return
        if hm and not self._ensure_hm_archive(download):
            return
        self._launch(download, season_key, target, hm, verify=True)
    ...
    def _pick_hm_archive(self) -> bool:
        ...
        if not picked:
            return False
        self._picked_archive = Path(picked)
        return True
    ...
    def _prepare(self, generation: int, enable_hm: bool,
        ...
        try:
            if pre is not None:
                pre()
            dd = _ensure_runtime(enable_hm, reporter)
            if dd is None:
                self._prepare_done_in.emit(generation, "", "Runtime setup failed")
                return
            self._prepare_done_in.emit(generation, str(dd), "")
        except RateLimited as e:
            self.rate_limited.emit(e.message())
            self._prepare_done_in.emit(generation, "", "Runtime setup failed")
    ...
    def _on_prepare_done_in(self, generation: int, dd: str, err: str) -> None:
        ...
        try:
            ...
            self.log_line.emit(log.fail("Setup failed", e))
            self._finish(1)
            return
        verb = "Verifying" if self._verifying else "Downloading to"
        self.log_line.emit(f"{verb} {self._target}")
    ...
    def _on_output(self) -> None:
        ...
        while True:
            ...
            if nl < 0:
                break
            lines.append(self._buffer[:nl].rstrip("\r"))
    ...
    def _check_prompt(self, text: str) -> None:
        ...
        self.log_line.emit(
            "Steam Guard code required" if kind == "guard" else "Steam password required"
        )
    ...
    def _on_process_finished(self, code: int, status: QProcess.ExitStatus) -> None:
        ...
        if status == QProcess.ExitStatus.CrashExit:
            self.log_line.emit("DepotDownloader stopped unexpectedly")
            self._finish(1)
            return
        cmd = self._commands[self._index]
        if code != 0:
            if not cmd["optional"]:
                self.log_line.emit(f"{cmd['name']} depot failed, exit code {code}")
                self._finish(code)
                return
    ...
    def _apply(self, generation: int, target: Path, download: dict, is_hm: bool,
        ...
        try:
            ...
            self.log_line.emit(log.fail("Install failed", e))
            ok = False
    ...
    def _switch_to_hm(self, season_key: str) -> None:
        ...
        target = folder.parent / variant_folder(season_key, True)
    ...
    def _switch_worker(self, generation: int, folder: Path, target: Path, download: dict,
        ...
        try:
            ...
            self.rate_limited.emit(e.message())
            self.log_line.emit("Switch failed")
            ok = False
        except Exception as e:
            self.log_line.emit(log.fail("Switch failed", e))
            ok = False
        self._apply_done_in.emit(generation, ok)
    ...
    def _finish(self, code: int) -> None:
        ...
        if self._cancelled:
            outcome = ""
            keep_active = bool(self._active_key) and not self._verifying
            ...
            if _out_of_space(self._target.parent):
                ...
                outcome = "verify_failed" if self._verifying else "failed"
            self._set_state("failed")
        else:
            outcome = "verified" if self._verifying else "done"
            if self._paused and self._enter_paused_from_queue():
                self._set_state("paused")
                self._set_verifying(False)
                self.active_key_changed.emit()
