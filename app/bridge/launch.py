...
class LaunchController(QObject):
    ...
    def launch(self, key: str, hm: bool) -> None:
        ...
        if is_game_running():
            ...
            threading.Thread(
                target=self._replace, args=(key, hm, folder), daemon=True
            ).start()
            return
        self._start(key, hm, folder)

    def _replace(self, key: str, hm: bool, folder: Path) -> None:
        stop_game(running_game_pids())
        self._start(key, hm, folder)

    def _fail(self, message: str) -> None:
        self._fail_in.emit(message)

    def _on_fail(self, message: str) -> None:
        self._set_launching(None)
        self.error.emit(message)

    def _start(self, key: str, hm: bool, folder: Path) -> None:
        launcher = folder / launcher_name(hm)
        if IS_WINDOWS:
            try:
                os.startfile(str(launcher), cwd=str(folder))
            except OSError as e:
                self._fail(log.fail("Launch failed", e))
            else:
                self._begin_launch(key, hm)
            return
        proton = resolve_proton(self._settings)
        if proton is None:
            self._fail(NO_PROTON)
            return
